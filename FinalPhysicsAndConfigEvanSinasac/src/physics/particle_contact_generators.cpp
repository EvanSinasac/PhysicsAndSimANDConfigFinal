			...
			for (unsigned int indexA = 0; indexA < particles.size() - 1; indexA++)
			{
				...
				for (unsigned int indexB = indexA + 1; indexB < particles.size(); indexB++)
				{
					...
					if (radiusA + radiusB > dist)
					{
						...
						// 3 - Box Entity
						// 4 - Spinning Blade
						if (contact->mParticle0->GetType() == 1 || contact->mParticle1->GetType() == 1)
						{
							contact->mRestitution = ::g_config.GetTargetRestitution();	// 0.7f;
						}
						else if (contact->mParticle0->GetType() == 2 || contact->mParticle1->GetType() == 2)
						{
							contact->mRestitution = ::g_config.GetHCJointRestitution();
						}
						else if (contact->mParticle0->GetType() == 3 || contact->mParticle1->GetType() == 3)
						{
							contact->mRestitution = ::g_config.GetBEJointRestitution();
						}
						else if (contact->mParticle0->GetType() == 4 || contact->mParticle1->GetType() == 4)
						{
							contact->mRestitution = ::g_config.GetSBRestitution();
						}
						else if (contact->mParticle0->GetType() == 5 || contact->mParticle1->GetType() == 4)
						{
							contact->mRestitution = ::g_config.GetTargetRestitution();
							if (contact->mParticle0->GetType() == 5)
							{
								srand(time(NULL));
								float randPosX = (rand() % 50) - 25.0f;
								contact->mParticle0->SetPosition(glm::vec3(randPosX,
									contact->mParticle0->GetPosition().y, contact->mParticle0->GetPosition().z));
							}
							else
							{
								srand(time(NULL));
								float randPosX = (rand() % 50) - 25.0f;
								contact->mParticle1->SetPosition(glm::vec3(randPosX,
									contact->mParticle1->GetPosition().y, contact->mParticle1->GetPosition().z));
							}
						}
						else if (contact->mParticle0->GetType() == 6 || contact->mParticle1->GetType() == 6)
						{
							contact->mRestitution = 0.01f;
						}
						else
						{
