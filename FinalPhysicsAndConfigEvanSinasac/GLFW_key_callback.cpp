...
/*static*/ void GLFW_key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (key == GLFW_KEY_1 && action == GLFW_PRESS)
    {
        ...
        velocity *= ::g_config.GetBulletVelocity();

        nPhysics::cParticle* p = new nPhysics::cParticle(::g_config.GetBulletMass(), glm::vec3(0.0f, 1.0f, -15.0f));
        p->SetVelocity(velocity);
        p->SetAcceleration(glm::vec3(0.0f, -9.8f, 0.0f));
        p->SetDamping(::g_config.GetBulletDamping());
        p->SetRadius(::g_config.GetBulletSize());
        //p->SetMass(radius * 10.0f);
        p->SetUseTimer(true);
        p->SetTimer(::g_config.GetBulletLifeSpan());
        g_world->AddParticle(p);
        ...
        newShot->positionXYZ = p->GetPosition();
        //newShot->orientationXYZ = glm::vec3(0.0f, 0.0f, 0.0f);
        newShot->scale = glm::vec3(::g_config.GetBulletSize());// 1.0f;
